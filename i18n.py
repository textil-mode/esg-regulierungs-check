...
ROLE_LABELS: dict[str, dict[str, str]] = {
    ...
    "Online-/Fernabsatz": {
        "de": "Online-/Fernabsatz",
        "en": "Online/distance selling",
        "es": "Venta en línea / a distancia",
        "fr": "Vente en ligne / à distance",
        "it": "Vendita online / a distanza",
        "zh": "线上 / 远程销售",
    },
}
...
FIELD_HELP: dict[str, dict[str, str]] = {
    "employees_total": {
        "de": "Alle Beschäftigten weltweit, Tochtergesellschaften eingerechnet; als Näherung genügt der Durchschnitt des letzten Geschäftsjahres. Der Begriff weicht je Gesetz ab: Lieferkettengesetz und HGB zählen Köpfe (Teilzeit als volle Person), die CSDDD rechnet Teilzeit anteilig als Vollzeitäquivalent. Das Formular hat nur ein Feld — geben Sie im Zweifel die höhere Zahl an, dann prüft das Werkzeug eher zu viel als zu wenig.",
        "en": "All employees worldwide, subsidiaries included; the average over the last financial year is a sufficient approximation. The term differs by law: the German Supply Chain Act and the Commercial Code count heads (part-time staff as one person each), while the CSDDD counts part-time staff pro rata as full-time equivalents. There is only one field — if in doubt, enter the higher figure so the check flags too much rather than too little.",
        "es": "Todas las personas empleadas en el mundo, filiales incluidas; basta con la media del último ejercicio. El concepto varía según la norma: la ley alemana de cadenas de suministro y el Código de Comercio cuentan personas (tiempo parcial como una persona), mientras que la CSDDD computa el tiempo parcial de forma proporcional. Solo hay un campo: en caso de duda indique la cifra más alta, así la verificación señala de más antes que de menos.",
        "fr": "Toutes les personnes employées dans le monde, filiales comprises ; la moyenne du dernier exercice suffit comme approximation. La notion varie selon les textes : la loi allemande sur les chaînes d'approvisionnement et le code de commerce comptent les personnes (temps partiel pour une personne entière), tandis que la CSDDD compte le temps partiel au prorata. Il n'y a qu'un seul champ : en cas de doute, indiquez le chiffre le plus élevé.",
        "it": "Tutte le persone occupate a livello mondiale, controllate comprese; come approssimazione basta la media dell'ultimo esercizio. La nozione varia a seconda della norma: la legge tedesca sulle catene di fornitura e il codice di commercio contano le persone (part-time come persona intera), mentre la CSDDD computa il part-time in misura proporzionale. Il campo è uno solo: nel dubbio indichi il valore più alto.",
        "zh": "全球全体员工，含子公司；取上一财政年度的平均值即可。不同法规口径不同：德国供应链法和商法典按人数计（兼职算一人），而 CSDDD 将兼职按比例折算为全职当量。此处只有一个字段——若不确定请填写较高的数字，宁可多提示，也不要漏掉。",
    },
    "employees_de": {
    ...
    "listed": {
        "de": "Gemeint ist die Kapitalmarktorientierung nach § 264d HGB: Ihr Unternehmen hat Wertpapiere ausgegeben, die an einem organisierten Markt gehandelt werden oder deren Zulassung beantragt ist — Aktien genügen, eine Anleihe aber ebenso. Beispiel: eine GmbH in Familienbesitz ist es nicht; eine GmbH, deren Anleihe an der Börse notiert ist, schon. Der Freiverkehr zählt nicht als organisierter Markt.",
        "en": "This refers to capital-market orientation under § 264d of the German Commercial Code: your company has issued securities traded on a regulated market, or has applied for their admission — shares qualify, but so does a single bond. Example: a family-owned GmbH is not capital-market oriented; a GmbH whose bond is listed on an exchange is. The open market (Freiverkehr) does not count as a regulated market.",
        ...
        "fr": "Il s'agit de l'orientation vers le marché des capitaux au sens du § 264d du code de commerce allemand : votre entreprise a émis des titres négociés sur un marché réglementé ou en a demandé l'admission — des actions, mais aussi un simple emprunt obligataire. Exemple : une GmbH détenue par une famille ne l'est pas ; une GmbH dont l'obligation est cotée en bourse, si. Le marché libre (Freiverkehr) ne compte pas comme marché réglementé.",
        "it": "Si intende l'orientamento al mercato dei capitali ai sensi del § 264d del codice di commercio tedesco: la sua azienda ha emesso titoli negoziati in un mercato regolamentato o ne ha chiesto l'ammissione; valgono sia le azioni sia una semplice obbligazione. Esempio: una GmbH a proprietà familiare non lo è; una GmbH la cui obbligazione è quotata in borsa sì. Il mercato non regolamentato (Freiverkehr) non conta come mercato regolamentato.",
